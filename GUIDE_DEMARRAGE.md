...
4. Ouvre le fichier `supabase/migrations/0001_init.sql` de ton projet (dans VS Code), copie tout son contenu, colle-le dans l'éditeur SQL de Supabase, et clique **Run**. Cela crée toutes les tables (recettes, commandes) et les règles de sécurité.
   Fais de même avec `supabase/migrations/0002_subscribers.sql`, `0003_orders_ledger.sql` puis `0004_orders_delivery_pickup.sql` (chacun dans une nouvelle requête SQL, dans l'ordre) : le premier ajoute la liste des amis inscrits aux alertes "nouveau stock disponible", le deuxième ajoute le suivi vendu/offert + prix sur les commandes, le troisième ajoute les statuts "livrée / à livrer" et "récupérée / à récupérer" sur chaque ligne de vente. À chaque futur ajout de fonctionnalité nécessitant la base de données, un nouveau fichier `000X_....sql` sera ajouté à exécuter de la même façon, toujours dans l'ordre des numéros — jamais besoin de retoucher les fichiers déjà exécutés.
...
- **Ventes** : le registre complet de toutes les commandes (celles passées sur le site + celles que tu ajoutes toi-même pour des ventes hors site, remises en main propre par exemple). Pour chaque ligne : recette, quantité, client, date, "vendu" ou "offert", montant encaissé si vendu, et deux statuts modifiables via le bouton **Modifier** : "à livrer" / "livrée" et "à récupérer" / "récupérée" (à livrer / à récupérer par défaut). La liste est triée par défaut du plus récent au plus ancien, et un menu **Filtrer par** permet de n'afficher que les commandes correspondant à un statut donné (à livrer, livrées, à récupérer, récupérées, vendues, offertes, commande site, saisie manuelle). Modifiable et supprimable à tout moment (utile pour nettoyer des essais/tests sans polluer le Suivi).
